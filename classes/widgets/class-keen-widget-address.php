...
if ( ! class_exists( 'Keen_Widget_Address' ) ) :
	 ...
	 * @since 1.0.0
	 */
	class Keen_Widget_Address extends WP_Widget {
		 ...
		 * @var object Class object.
		 */
		public $id_base = 'keen-widget-address';
		...
		function __construct() {
			parent::__construct(
				$this->id_base,
				__( 'Keen: Address Info', 'keen-widgets' ),
				array(
					'classname'   => $this->id_base,
					'description' => __( 'Display Address.', 'keen-widgets' ),
				),
				array(
		...
		function register_scripts() {
			...
			wp_register_style( 'keen-widgets-' . $this->id_base, $css_uri . 'keen-widget-address' . $file_prefix . '.css' );
		...
		function widget( $args, $instance ) {
			...
			if ( function_exists( 'keen_get_option' ) ) {
				$widget_content_font_size = keen_get_option( 'font-size-widget-content' );
		...
		function update( $new_instance, $old_instance ) {
			 ...
			 *       For now we have not able to detect the `checkbox` field in `generate` of class `Keen_Widgets_Helper`.
			 */
		...
		function form( $instance ) {
			...
			$fields = array(
				...
				array(
					...
					'name'    => __( '	Space Between Address Fields:', 'keen-widgets' ),
					'unit'    => 'Px',
